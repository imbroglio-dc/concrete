...
List updateHazardsCpp(List Hazards,
  ...
  for (int l = 0; l < L; l++) {
    ...
    for (int j = 0; j < TargetEvent.size(); j++) {
      ...
      // Pre-scale NuisanceWeight once per event (by column)
      arma::mat scaledNW = NuisanceWeight;
      scaledNW.each_row() %= GStar.t();  // GStar is N x 1
      ...
      for (int k = 0; k < TargetTime.n_elem; k++) {
        ...
        // Throw error if no PnEIC row found
        if (!foundPn) {
          stop("PnEIC value not found for Time = " + std::to_string(tau) +
            " and Event = " + jname);
        }
        
        // Find index of tau in EvalTimes
        arma::uvec tau_idx = arma::find(EvalTimes == tau, 1);
        if (tau_idx.n_elem == 0) continue; // tau not in EvalTimes
        int t_tau = tau_idx(0);
        ...
        arma::mat hFS = Fjt;                           // copy (T×N)
        hFS.each_row() = F_tau;                        // broadcast rowvec F_tau across rows
        hFS -= Fjt;                                    // elementwise subtract
        hFS /= TotalSurv;                              // elementwise divide
        hFS.rows(arma::find(EvalTimes > tau)).zeros(); // zero rows after tau
        
        // Inline clever covariate calculation
        arma::mat ClevCov = scaledNW % ( (lname == jname ? 1.0 : 0.0) - hFS );
        
        // Update hazard increment (in-place accumulation)
        update_l += ClevCov * pn;
      }
    }
