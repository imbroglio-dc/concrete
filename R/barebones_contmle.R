...
doConCRTmle <- function(EventTime, EventType, Treatment, CovDataTable, Time = NULL,
                        ID = seq_along(EventTime), TargetTimes = sort(unique(EventTime)),
                        TargetEvents = NULL, Models, CVArgs = NULL, NumUpdateSteps = 30,
                        OnestepEps = 0.5, Verbose = FALSE, ...)
{
  # parameter checking --------------------------------------------------------------------------
  if (any(c("ID", "Time", "Event", "Trt", "t") %in% colnames(CovDataTable)))
    stop(paste0("'ID', 'Time', 'Event', 'Trt', and 't'",
                "are reserved column names. Rename covariate",
                "column names to avoid name collisions.\n"))


  if (!all(sapply(list(EventTime, EventType, Treatment, Time), is.numeric)))
    stop(paste0("EventTime, EventType, Treatment, and Time ",
  ...
  strata_ids <- as.numeric(factor(paste0(data$Trt, ":", data$Event)))
  ...
  Ht <- rbind(data, copy(data)[, Trt := 1 - Trt])[order(ID, Trt)]
  setcolorder(Ht, c("ID", "Trt", "Time", "Event"))

  ## clever covariate A component ----
  Ht[Trt == 1, Ht_a := 1 / propensity]
  Ht[Trt == 0, Ht_a := 1 / (1 - propensity)]
  ...
  setnames(Ht, "Trt", "A") # A is the target trts, Trt is the assigned
  setnames(Ht, "Time", "T_tilde")
  Ht <- merge(data[, c("ID", "Trt")],
              Ht,
              by = "ID")
  Ht_colnames <- c("ID", "Trt", "A", "T_tilde", "Event", "Ht_a",
  ...
  for (k in target_times) {
    # Ht[, Ht.g := Ht.g * (Time <= k) * (Time <= T_tilde) * (Trt == A)]
    for (j in target_Events) {
