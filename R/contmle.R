...
#'
#' @param dt
#' @param estimation outcome model
#' @param target when there are competing risks, what is the target? if 0, only survival is targeted
#' @param hal.screening use poisson-based or cox-based hal?
#' @param one.step use iterative or one-step tmle; (for competing risks, one-step is default)
#' @param deps.size
#' @param no.small.steps
#' @param push.criterion
#' @param iterative
#' @param no.update.if.solved option to not update in directions currently already solved
#' @param separate.cr target cause-specific hazards separately or together in one-step tmle
#' @param simultaneous.ci simultaneous inference
#' @param treat.model treatment model
#' @param cut.off.cens cut-off for truncation of censoring weights
#' @param weighted.norm use weighted norm in multivariate one-step
#' @param treat.effect treatment effect of interest
#' @param pi.star.fun specify stochastic intervention
#' @param tau time-point(s) of interest
#' @param use.observed.times increasing grid?
#' @param length.times
#' @param check.times.size
#' @param delta.min
#' @param check.min
#' @param check.max
#' @param sl.method pick super learning loss (should not change this)
#' @param V number of folds in cross-validation
#' @param lambda.cv specify penalization in hal?
#' @param maxit
#' @param init.lambda.cvs specify grid over which to pick penalization in hal by cross-validation
#' @param lambda.cvs
#' @param lambda.grid.size
#' @param penalize.time penalize time indicators in hal?
#' @param cut.covars pick grid for indicators in hal
#' @param cut.time pick grid for indicators in hal
#' @param cut.time.A pick grid for indicators in hal
#' @param cut.L.A pick grid for indicators in hal
#' @param cut.L.interaction pick grid for indicators in hal
#' @param maxIter maximum number of iterations in iterative tmle
#' @param verbose
#' @param verbose.sl
#' @param check.sup
#' @param output.km for comparison; output kaplan-meier and hr
#' @param only.km for comparison; output kaplan-meier and hr
#' @param only.cox.sl for comparison; output kaplan-meier and hr
#' @param output.RR
#' @param sl.change.points models incorporated in super learner
#' @param sl.models models incorporated in super learner
#'
#' @return
#' @export
#'
...
contmle <- function(dt,
                    ...
                    sl.models = list(
                      mod1 = list(Surv(time, delta == 1) ~ A + L1 + L2 + L3),
                      mod2 = list(Surv(time, delta == 1) ~
                                    A + L1.squared + L2 + L3),
                      mod3 = list(Surv(time, delta == 1) ~
                                    L2.squared + A + L1.squared + L2 + L3),
                      mod4 = list(Surv(time, delta == 1) ~
                                    A + L1.squared),
                      mod5 = list(Surv(time, delta == 1) ~
                                    A * L1 + L2 + L3),
                      mod6 = list(Surv(time, delta == 1) ~
                                    A * L1.squared + L2 + L3)
                    )) {
  # 0. Setup --------------------------------------------------------------------------------
  ...
  ## 0.3 Check if multiple models are for a single event type ------------------------------
  ...
  for (each in 1:length(estimation))
  {
    ...
    if (fit[1] == "sl" | fit[1] == "cox.hal.sl") {
      if (verbose)
        print(paste0("use sl for ", fit.name))
      ...
      if (verbose.sl)
        print(paste0("model picked for ", fit.name, ": "))
      if (verbose.sl)
        print(fit.model)
      ...
      if (any(names(sl.pick$picked.cox.model) == "change.point")) {
        ...
        if (fit.changepoint > 0) {
          if (verbose.sl)
            print(paste0("changepoint picked: ", fit.changepoint))
      ...
      if (fit[1] %in% c("km")) {
        ...
        estimation[[each]]$model <-
          fit.model <- formula(paste0(tmp.model[2], tmp.model[1], tmp.model[3]))
    ...
    if (fit[1] == "km") {
      ...
      if (fit[1] == "sl" & length(grep("coxnet", sl.pick)) > 0) {
        ...
        ### 2.4.3 Baseline hazard if event is modeled using Cox? -----------------------
        bhaz.cox <- merge(
          bhaz.cox,
          rbind(
            data.table(time = 0, hazard = 0),
            suppressWarnings(setDT(basehaz(
  ...
  if (output.km) {
    if (cr & !target.S) {
      km.mod <- paste0(gsub(estimation[[1]][["event"]], "",
                       ...
                       "~", A.name)
      km.fit <- summary(
        prodlim(formula(km.mod), data = dt),
        cause = target,
        times = tau,
        asMatrix = TRUE
      )$table
      if (length(a) == 1) { # treatment specific risk / survival, i.e. single binary treatment level
        km.est <-
          as.numeric(km.fit[km.fit[, 2] == paste0(A.name, "=", a), , drop = FALSE][, "cuminc"])
        km.se <-
          as.numeric(km.fit[km.fit[, 2] == paste0(A.name, "=", a), , drop = FALSE][, "se.cuminc"])
      } else { # ate
        km.est <-
          as.numeric(km.fit[km.fit[, 2] == paste0(A.name, "=", "1"), , drop = FALSE][, "cuminc"]) -
          (as.numeric(km.fit[km.fit[, 2] == paste0(A.name, "=", "0"), , drop = FALSE][, "cuminc"]))
        km.se <-
          sqrt((as.numeric(km.fit[km.fit[, 2] == paste0(A.name, "=", "1"), ,
                                  drop = FALSE][, "se.cuminc"])) ^ 2 +
                 (as.numeric(km.fit[km.fit[, 2] == paste0(A.name, "=", "0"), ,
                                    drop = FALSE][, "se.cuminc"])) ^ 2)
      }
    } else {
      if (target.S) { # if the target is event-free survival
        km.mod <- paste0("Hist(", time.var, ",", delta.var, "!=0)", "~", A.name)
      } else {
        km.mod <- paste0(gsub("Surv", "Hist",
                              as.character(estimation[[1]][["model"]])[2]), "~", A.name)
      }
      ...
      if (length(a) == 1) { # treatment specific risk / survival, i.e. single binary treatment level
        ...
        if (cr) {
          km.est <-
            as.numeric(km.fit[km.fit[, 1] == paste0(A.name, "=", "1"), , drop = FALSE][, "surv"]) -
            as.numeric(km.fit[km.fit[, 1] == paste0(A.name, "=", "0"), , drop = FALSE][, "surv"])
        } else {
          km.est <-
            1 - as.numeric(km.fit[km.fit[, 1] == paste0(A.name, "=", "1"), , drop = FALSE][, "surv"]) -
            (1 - as.numeric(km.fit[km.fit[, 1] == paste0(A.name, "=", "0"), , drop = FALSE][, "surv"]))
        }
        km.se <- sqrt((as.numeric(km.fit[km.fit[, 1] == paste0(A.name, "=", "1"), ,
                                         drop = FALSE][, "se.surv"])) ^ 2 +
                        (as.numeric(km.fit[km.fit[, 1] == paste0(A.name, "=", "0"), ,
                                           drop = FALSE][, "se.surv"])) ^ 2)
      }
    }
  }
  ...
  if (only.km)
    return(lapply(1:length(target), function(each.index) {
      out <-
        rbind(km.est = km.est[((each.index - 1) * length(tau) + 1):(each.index * length(tau))],
              km.se = km.se[((each.index - 1) * length(tau) + 1):(each.index * length(tau))])
      colnames(out) <- paste0("tau=", tau)
      return(out)
    }))

  ## prep variable for HAL - can this be moved down to the rest of HAL later? ---------------
  any.hal <- unlist(lapply(estimation, function(each) length(grep("hal", each[["fit"]])) > 0))

  if (!any(any.hal))
    bhaz.cox <- bhaz.cox[get(time.var) <= max(tau)]

  ## ?.? add subject-specific information - what does this do? -------------------------------
  # is this just adding cause-specific cox hazards?
  ...
  for (each in 1:length(estimation)) {
    if (estimation[[each]][["fit"]][1] == "sl" &
        length(grep("coxnet", estimation[[each]][["sl.pick"]])) > 0) { # if using sl coxnet
  ...
  if (verbose)
    paste0("min of censoring weights: ", mat[, min(surv.C1)])


  ## 4.? poisson HAL initial estimate -----------------------------------------------------
  if (any(any.hal)) {
    set.seed(13444)
    count.hals <- 1
    ...
    for (each in (1:length(estimation))[any.hal]) {
      ...
      if (any(estimation[[each]]$fit == "cox.hal") |
          any(estimation[[each]]$fit == "cox.hal.sl")) {
        # only testing - what does this do? -----------------------------------------
        ...
        if (hal.screening) { # first screening
          ...
          (
            two.way.screening <-
              hal.screening(
                covars = one.way.screening,
                dt = dt,
                cut.one.way = 5,
                delta.var = "delta",
                delta.value = fit.delta,
                treatment = "A",
                cut.time.treatment = 3,
                order = 2,
                mat = mat
              )
          )
          ...
          if (verbose &
              length(one.way.screening) > 0)
            print(paste0(
              "variables picked by initial screening: ", paste0(one.way.screening, collapse = ", ")
            ))
          ...
          mat <-
            fit.hal(
              ...
              verbose = verbose,
              predict = max(tau),
              treatment.prediction = "A",
              treatment = "A"
            )

          if (FALSE) {
            diff(mat[time <= tau, exp(-cumsum(dhaz1 * fit.cox1))[.N],
                     by = c("id", A.name)][, mean(V1), by = A.name][order(V1), V1])
        ...
        if (hal.screening) {
          ...
          if (length(one.way.screening) > 0) {
            ...
            if (verbose &
                length(one.way.screening) > 0)
              print(paste0(
                "variables picked by initial screening: ",
                paste0(one.way.screening, collapse = ", ")
              ))
          }
  ...
  ## 4.2 truncate censoring weights if necessary (and report if done) ----------------------

  if (mat[, min(surv.C1) < cut.off.cens]) {
    if (mat[get(time.var) <= time.obs, min(surv.C1) < cut.off.cens]) {
      not.fit.list[[length(not.fit.list) + 1]] <-
        paste0(
          paste0("there seems to be positivity issues for censoring; truncated ",
                 sum(mat[get(time.var) <= time.obs,
                         sum(unique(surv.C1 < cut.off.cens)), by = "id"][, 2][[1]]),
                 " observation(s), at level ", cut.off.cens)
        )
    }
  ...
  if (mat[, min(prob.A) < cut.off.cens]) {
    if (mat[, min(prob.A) < cut.off.cens]) {
      not.fit.list[[length(not.fit.list) + 1]] <-
        paste0(
          paste0(
            "there seems to be positivity issues for treatment; truncated ",
            sum(mat[, sum(unique(prob.A < cut.off.cens)), by = "id"][, 2][[1]]),
            " observation(s), at level ", cut.off.cens
          )
        )
    }
    mat[prob.A <= cut.off.cens, Ht := Ht * (prob.A ^ get(A.name) *
                                              (1 - prob.A) ^ {1 - get(A.name)}) / cut.off.cens]
    mat[prob.A <= cut.off.cens, prob.A := cut.off.cens]
  }


  ## 4.3 compute clever covariates --------------------------------------------------------
  # possible to  move the clever covariate intervention part down here?
  ...
  # append surv = 0 to the start of every subject's survival prob vector why? ---------
  ...
  if (cr) {
    for (each in outcome.index) { # looping over each target event j
      ...
      mat[, (paste0("F", fit.delta, ".t")) := cumsum(surv.t * get(paste0("dhaz", fit.delta)) *
                                                       get(paste0("fit.cox", fit.delta))),
          by = c("id", A.name)]
      for (kk in 1:length(tau)) { # cause-specific, conditional CIF at target times
    ...
    if (target.S) { # clever covariate for target parameter of event-free survival
    ...
    for (kk in 1:length(tau)) {
      ...
      mat[surv.t == 0, (paste0("Ht.lambda.", kk)) := 1]  # what does this do? ---------------
    }
  }

  ### 4.3.2 Initial estimate of marginal cause-specific CIFs (subdistributions)
  if (cr) {
    if (treat.effect[1] == "stochastic") {
    ...
    } else { # Mean cause-specific risk, at target time(s)
      init.fit <- lapply(target, function(each) {
        sapply(1:length(tau), function(kk) {
          mean(rowSums(sapply(a, function(aa)
            (
              2 * (aa == a[1]) - 1
            ) * (mat[get(A.name) == aa, get(paste0("F", estimation[[outcome.index[each]]][["event"]],
                                                   ".tau", kk))[1],
                     by = "id"][, 2][[1]]))))
        })
      })
    ...
    if (treat.effect[1] == "stochastic") {
      ...
      init.fit <- sapply(1:length(tau), function(kk) {
        mean(rowSums(sapply(a, function(aa)
          (2 * (aa == a[1]) - 1) *
            (mat[get(A.name) == aa, 1 - get(paste0("surv.tau", kk))[1], by = "id"][, 2][[1]]))))
      })
    }
  }
  ...
  ## 4.4 EIC ---------------------------------------------------------------------------
  if (cr) { ### 4.4.1 function to evaluate influence function with competing risks ---------
    eval.ic <-
      function(mat,
               fit,
          ...
          sapply(1:length(tau.values), function(kk) { # loop over target times
            ...
            if (treat.effect[1] == "stochastic") {
              ic.squared <- (out + rowSums(sapply(a, function(aa)
                (mat[get(A.name) == aa, pi.star[1] *
                       get(paste0("F", fit.delta,".tau", k2))[1],
  ...
  } else { ### 4.4.2 function to evaluate influence function without competing risks ---------
    eval.ic <-
      function(mat,
               fit,
               target.index = 1,
            ...
            ic.squared <- (out[, 2][[1]] +
                             rowSums(sapply(a, function(aa)
                               (2 * (aa == a[1]) - 1) * (mat[get(A.name) == aa,
                                                             ...
                                                             by = "id"][, 2][[1]]))) -
                             (length(a) == 2) * fit[kk] - (length(a) == 1) * (1 - fit[kk]))
          }
          if (Sigma)
            return(ic.squared)
          else
    ...
    if (target.S) {
      Sigma <-
        eval.ic(
          mat,
          init.fit,
          target.index = outcome.index[target],
          Sigma = TRUE,
          survival = TRUE
        )
    } else {
  ...
  if (cr) {
    ...
    if (output.km) {
      km.list <-
        lapply(1:min(length(target), length(tmle.list$init)), function(each.index) {
          out <-
            rbind(km.est = km.est[((each.index - 1) * length(tau) + 1):(each.index * length(tau))],
                  km.se = km.se[((each.index - 1) * length(tau) + 1):(each.index * length(tau))])
          colnames(out) <- paste0("tau=", tau)
  ...
  ### 4.4.4 functions to compute EICs... again? -------------------------------------------
  # are these functions redundant with 4.4.2 and 4.4.3? need to check ---------------------
  if (cr) {
    eval.equation <- function(mat,
      ...
      outer <- lapply(target.index, function(each) { # loop over target events (j)
        ...
        sapply(1:length(tau.values), function(kk) { # loop over target times (k)
          ...
          # or sort the tau's and simplify to sum(tau.all <= tau.values[kk])

          out <- 0
          for (each2 in cr.index) { # loop over all events (l)
            fit.delta2 <- estimation[[each2]][["event"]]
    ...
    eval.equation <-
      function(mat,
        ...
        sapply(1:length(tau.values), function(kk) {
          k2 <- (1:length(tau.all))[tau.all == max(tau.all[tau.all <= tau.values[kk]])]
          out <- mat[(get(time.var) <= tau.values[kk]),
                     sum((get(A.name) == A.obs) * (get(time.var) <= time.obs) * Ht *
                           get(paste0("Ht.lambda.", k2)) *
                           ((delta.obs == 1 & get(time.var) == time.obs) -
                              exp(eps * Ht * get(paste0("Ht.lambda.", k2))) * dhaz1 * fit.cox1
                           )),
                     by = "id"]
          return(mean(out[, 2][[1]]))
        })
  ...
  if (check.sup & weighted.norm[1] == "Sigma" & FALSE) {
    print("-----")
    print(c(
      matrix(eval.equation(mat, eps = 0), nrow = 1) %*%
        Sigma.inv %*% matrix(eval.equation(mat, eps = 0), ncol = 1)
    ))
    print(c(
      matrix(eval.equation(mat, eps = 0), nrow = 1) %*%
        diag(1 / (init.ic ^ 2 * n)) %*% matrix(eval.equation(mat, eps = 0), ncol = 1)
    ))
    print(
      nrow(Sigma.inv) * c(
        matrix(eval.equation(mat, eps = 0), nrow = 1) %*%
  ...
  # 5. Perform one-step TMLE update -------------------------------------------------

  if ((length(tau) > 1 &
       !iterative) | one.step | (length(target) > 1 & !iterative)) {
    second.round <- FALSE
    ...
    if (weighted.norm[1] == "sigma") {
      if (target.S) {
        ...
        Pn.eic2.fun <-
          function(Pn.eic)
            Pn.eic / (ifelse(any(unlist(S.se.init) == 0), S.se.init + 0.00001, S.se.init)^2 * n)
      } else {
      ...
      if (target.S) {
        Pn.eic2.fun <-
          function(Pn.eic)
            as.numeric(matrix(unlist(Pn.eic), 1, length(unlist(Pn.eic))) %*% Sigma.inv)
      } else {
        Pn.eic2.fun <- function(Pn.eic) {
          Pn.eic.tmp <-
            as.numeric(matrix(unlist(Pn.eic), 1, length(unlist(Pn.eic))) %*% Sigma.inv)
          if (length(target) > 1) {
            return(lapply(1:length(target), function(kk)
              Pn.eic.tmp[(1 + (kk - 1) * length(tau)):(kk * length(tau))]))
          } else {
            return(Pn.eic.tmp)
          }
        }
    ...
    for (step in 1:no.small.steps) {
      ...
      if (cr) {
        if (target.S) {
          ...
          if (!separate.cr) {
            for (each in outcome.index) { # loop over all event hazards
              fit.delta <- estimation[[each]][["event"]]
      ...
      if (verbose)
        print(paste0("target level = ", criterion))
      if (verbose)
        print(paste0("max eic = ", max(abs(
          unlist(Pn.eic3)
        ))))
    ...
    update.fit <- lapply(target2, function(target1) {
        ...
        for (iter in 1:maxIter) {
          if (FALSE) {
            ...
            lines(sapply(seq(-1, 1, length = 100), function(eps)
              sum(unlist(
                eval.equation(mat, eps, target.index = outcome.index[target1])
              )[tau == tt])))
          }
          ...
          if (cr) {
            ...
            for (kk in (1:length(tau))[tau == tt]) {
              for (each in outcome.index) {
                fit.delta <- estimation[[each]][["event"]]
                index <-
                  (1:length(outcome.index))[outcome.index == each]
                mat[, (paste0("fit.cox", fit.delta)) :=
                      get(paste0("fit.cox", fit.delta)) *
                      exp(eps.hat[index] * Ht *
                            rowSums(sapply(target1, function(target11)
                              get(
                                paste0("Ht", target11, ".lambda", fit.delta, ".", kk)
                              ))))]
                mat[get(paste0("fit.cox", fit.delta)) > 500, (paste0("fit.cox", fit.delta)) := 500]
                mat[, surv.t := surv.t * exp(-cumsum(get(
                  paste0("dhaz", fit.delta)
                ) *
                  get(
                    paste0("fit.cox", fit.delta)
                  ))),
                by = c("id", A.name)]
                mat[get(paste0("fit.cox", fit.delta)) == Inf, surv.t := 0]
              }
            ...
            for (each in outcome.index[target1]) {
              fit.delta <- estimation[[each]][["event"]]
              mat[, (paste0("F", fit.delta, ".t")) := cumsum(surv.t *
                                                               get(paste0("dhaz", fit.delta)) *
                                                               get(paste0("fit.cox", fit.delta))),
                  by = c("id", A.name)]
              for (kk in (1:length(tau))[tau == tt]) {
                mat[, (paste0("F", fit.delta, ".tau", kk)) :=
          ...
          if (length(target1) == 1) {
            eval.iter <- abs(sum(unlist(
              eval.equation(mat, 0, target.index = outcome.index[target1])
            )[tau == tt]))
            if (is.list(init.ic))
              rhs <- (init.ic[[target1.index]][tau == tt] *
                        sqrt(n)) / (sqrt(n) * log(n))
            else
              rhs <- (init.ic[tau == tt]#[[target1.index]]
                      *
                        sqrt(n)) / (sqrt(n) * log(n))
          } else {
            eval.iter <- abs(sum(sapply(1:length(outcome.index), function(target1) {
              unlist(eval.equation(mat, 0, target.index = outcome.index[target1]))[tau == tt]
            })))
            if (is.list(init.ic))
              rhs <- sqrt(mean(rowSums(sapply(target1, function(target11) {
                unlist(
                  eval.ic(
                    mat,
                    init.fit,
          ...
          if (eval.iter <= rhs | iter == maxIter) {
            ...
            if (cr) {
              ...
              if (treat.effect[1] == "stochastic") {
                update.est <- sapply((1:length(tau))[tau == tt], function(kk) {
                  mean(rowSums(sapply(a, function(aa)
                    (
                      mat[get(A.name) == aa, pi.star[1] * (1 - get(paste0("surv.tau", kk))[1]), by =
                            "id"][, 2][[1]]
                    ))))
                })
              } else {
                update.est <- sapply((1:length(tau))[tau == tt], function(kk) {
                  mean(rowSums(sapply(a, function(aa)
                    (2 * (aa == a[1]) - 1) * (mat[get(A.name) == aa, 1 -
                                                    get(paste0("surv.tau", kk))[1], by = "id"][, 2][[1]]))))
                })
              }
            }
            ...
            if (length(target1) > 1 | target.S) {
              ...
              update.ic <-
                sqrt(mean(rowSums(
                  sapply(target1, function(target11) {
                    unlist(
                      eval.ic(
                        mat,
                        update.est,
            ...
            out.list <-
              list(update.est = update.est, update.ic = update.ic)
            if (cr) {
              if (length(target1) == 1) {
      ...
      update.list.inner <-
        do.call("cbind", lapply(1:length(update.fit.inner), function(each.index) {
  ...
  if (simultaneous.ci) {
    ...
    # plot(unlist(lapply(1:length(generate.max), function(jj)
    #   quantile(abs(unlist(generate.max[1:jj])), p=0.95))))
  ...
  if (FALSE) {
    ...
    1 - sum(unlist(lapply(tmle.list$tmle.second.round, function(xx)
      xx["tmle.est", ])))
    sqrt(sum(unlist(
      lapply(tmle.list$tmle.second.round, function(xx)
        xx["tmle.se", ] ^ 2)
    )))
    update.ic <-
      sqrt(mean(rowSums(sapply(1:length(outcome.index), function(target11) {
        unlist(
          eval.ic(
            mat,
            unlist(lapply(tmle.list$tmle.second.round, function(xx)
              xx["tmle.est", ])),
            target.index = outcome.index[target11],
            tau.values = tau,
            survival = TRUE
  ...
  if (length(check.times.size) > 0) {
    ...
    all.fit <- sapply(1:length(unique.times2), function(kk) {
      mean(rowSums(sapply(a, function(aa)
        (2 * (
          aa == a[1]
        ) - 1) * (mat[get(A.name) == aa, 1 - get(paste0("surv.tau", kk))[1], by =
                        "id"][, 2][[1]]))))
    })
    ...
    Pn.eic.all4 <-
      lapply(1:length(Pn.eic.all), function(kk)
        Pn.eic.all[[kk]] / (ifelse(
          any(unlist(all.ic) == 0), all.ic[[kk]] + 0.001, all.ic[[kk]]
        ) * sqrt(n)))
    check.sup.norm.all <-
      abs(unlist(Pn.eic.all4)) <= 1 / (sqrt(n) * log(n))
    # criterion#/ifelse(push.criterion, 1, sqrt(length(target)*length(tau)))
