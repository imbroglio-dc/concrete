...
W <- read_sas(paste0(data_path, "adtte.sas7bdat")) %>% dplyr::select(USUBJID) %>% distinct() %>%
    left_join(., W)

W <- W %>% mutate_if(is.character, as.factor)

# REMOVE UNITS, ID VARIABLES, AND POST-BASELINE VARIABLES
W <- dplyr::select_at(W, vars(!ends_with("U"))) %>% # remove units
    ...
    dplyr::select_at(vars(!starts_with("TRDU"))) # treatment duration (post-baseline)
