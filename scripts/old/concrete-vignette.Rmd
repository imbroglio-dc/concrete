...
---
# What is concrete:
Package implementing one-step TMLE for survival and competing risks in continuous time.
- Data Structure
    - binary treatment given at baseline (support for multinomial and continous to come)
    - right-censored time-to-event outcomes, possibly with competing risks, in continuous time
    - covariates measured at baseline
    - treatment and outcomes cannot have missingness. Missingness in covariates should be pre-imputed.
...
- Estimands
    ...
    - can target risks at multiple times and multiple cause-specific risks for competing risks
    
- Estimation
    - propensity scores: `SuperLearner` or `sl3`-based SuperLearner
    - hazards: Cox-based discrete SuperLearner (Coxnet and HAL to come)
    - one-step TMLE based on the local PnEIC approximation
    - g-formula plug-in estimator using the discrete Superlearner hazards
    - IC-based inference for TMLE
...
# concrete Workflow
1. Cycle inputs through formatArguments() until satisfied with estimation specification
2. run doConcrete()
3. run getOutput() for plottable outcome tables

# Usage: pbc minimal example
...
# treatment cannot have missingness (and currently has to be binary 0,1)
...
### DataTable
 ...
 - necessary columns: 
    ...
    - Treatment: the intervention variable - binary is fully supported while continous and multinomial interventions may be supported in the future.
 - optional columns:
...
ConcreteArgs <- formatArguments(Data = obs, 
                                EventTime = "time", 
                                EventType = "status", 
                                Treatment = "trt", 
                                ID = "id", 
...
ConcreteArgs <- formatArguments(Data = obs, EventTime = "time", EventType = "status", 
                                Treatment = "trt", ID = "id", 
