...
synthesize.formula <- function(object, # a formula object Surv(time,event) or Hist(time,event)
    ...
    tt <- all.vars(stats::update(object,".~1"))

    # covariates
    vv <- all.vars(stats::formula(stats::delete.response(stats::terms(object))))
    fml <- paste(vv,collapse = "+")
    vv.withtrans <- attr(stats::terms(stats::formula(stats::delete.response(stats::terms(object)))),"term.labels")
    ...
    object <- stats::update(object,stats::as.formula(paste0("~",fml)))
    ...
    if (recursive){
      ...
      while (length(cv) > 1) {
        ...
        regression(object) <- stats::as.formula(paste0(cv[1],"~",paste(cv2,collapse = "+")))
        cv <- cv2
      }
    }
    # outcome
    ...
    if (length(tt)==1){
        ...
        et.formula <- stats::formula(paste0(tt[[1]]," ~ min(",paste(paste0("time.event.",events,"=",events),collapse=", "),")"))
...
synthesize.lvm <- function(object,
    ...
    for(var in colnames(data)[grepl('factor|character', sapply(data, class))]){
        ...
        if(!any(grepl(var,  dimnames(object$M)[[1]]))){
            ...
            for(lvl in levels(data[[var]])[-1]){
                ...
                formula <- stats::as.formula(paste0(var, lvl,"~",var))
    ...
    if (has.eventTime){
        ...
        for (latvar in object$attributes$eventHistory[[timename]]$latentTime){
            ...
            latvar_formula <- stats::as.formula(paste0("~", latvar))
            ...
            surv_formula <- stats::as.formula(paste0("Surv(", response, ")~", covariates))
            if (class(status_ind) != "numeric") {stop("event or status variable has to be numeric")}
            G <- survival::survreg(surv_formula, data = data)
            reg_formula <- stats::as.formula(paste0(latvar, "~", covariates))
            lava::distribution(sim_model, latvar_formula) <- lava::coxWeibull.lvm(scale=exp(-stats::coef(G)["(Intercept)"]/G$scale),shape=1/G$scale)
            lava::regression(sim_model, reg_formula) <- -stats::coef(G)[-1]/G$scale
        ...
        if (!fromFormula){
          ...
          et.formula <- stats::formula(paste0(timename," ~ min(",paste0(cens,"=",events,collapse=", "),")"))
          sim_model <- lava::eventTime(sim_model,et.formula, object$attributes$eventHistory[[timename]]$names[2])
        }
        else {
          et.formula <- stats::formula(paste0(timename," ~ min(",paste(paste0("time.event.",events,"=",events),collapse=", "),")"))
          sim_model <- lava::eventTime(sim_model,et.formula, object$attributes$eventHistory[[timename]]$names[2])
        }
    }
    ...
    for(var in latent_vars[!(latent_vars %in% object$attributes$eventHistory$time$names)]){
        ...
        reg_formula <- stats::as.formula(paste0(var, "~", covariates))
        ...
        if ("binomial" %in% attributes(object$attributes$distribution[[var]])$family){
            fit <- stats::lm(reg_formula,data=data,family="binomial")
            p0<-exp(stats::coef(fit)[1])/(1+exp(stats::coef(fit)[1]))
            lava::distribution(sim_model,stats::as.formula(paste0("~", var))) <- lava::binomial.lvm(p=p0)
            lava::regression(sim_model,reg_formula)<-stats::coef(fit)[-1]
        }
        #case: categorical
        else if (var %in% dichotomized_variables){
          if (verbose) warning("Synthesize untested for categorical variables")
          for(lvl in levels(data[[var]])[-1]){
            reg_formula <- stats::as.formula(paste0(var, lvl, "~", covariates))
            fit <- stats::lm(reg_formula,data=data,family="binomial")
            p0<-exp(stats::coef(fit)[1])/(1+exp(stats::coef(fit)[1]))
            lava::distribution(sim_model,stats::as.formula(paste0("~", var))) <- lava::binomial.lvm(p=p0)
            lava::regression(sim_model,reg_formula)<-stats::coef(fit)[-1]
        ...
        # case: gaussian
            ...
            fit <- stats::lm(reg_formula,data=data)
            lava::regression(sim_model,reg_formula)<-stats::coef(fit)[-1]
            lava::distribution(sim_model,stats::as.formula(paste0("~", var))) <- lava::normal.lvm(mean = stats::coef(fit)[1], sd = summary(fit)$sigma)
        }
    ...
    for (v in logtrans){
        transform(sim_model,stats::as.formula(paste0(v,"~log",v))) <- function(x){exp(x)}
    }
