...
source("simPBC.R")
set.seed(0)
data <- as.data.table(simPBC(n = 1e7))
data <- data[status != 0, ]
data[, Time := pmin(time.transplant, time.death, 3000)]
data[, Event := (1 + (time.transplant > time.death)) * (Time >= 3000)]
data <- data[Event != 0, ]
risks <- do.call(rbind, lapply(seq(0, 3000, 200), function(x) data[, list("x" = x, y = mean(Time <= x)), by = c("Event", "trt")]))
```
...
ConcreteArgs$MaxUpdateIter <- 500
