...
What concrete does: 
- Data Structure
 - continuous time-to-event outcomes + right-censored + competing risks
...
- Estimands
 ...
 - 
- TMLE
 - one-step TMLE using local PnEIC-clever covariate approximation

What concrete does not do: 
- longitudinal treatment, mediation, time-dependent confounding 
- delayed entry, left-censoring
- 

concrete workflow
 - cycle inputs through formatArguments() until it returns no errors
 - run doConcrete() and wait
 - get output from getOutput() (pretty print and plot not yet implemented)
...
### DataTable
DataTable should be a data.frame or data.table, each row containing one subject's information (longitudinal data not supported). DataTable must include the following columns: 
\begin{itemize}
\item EventTime: the time to observed failure (or censoring) event.
\item EventType: the type of event, encoded as integers with 0 being reserved for right-censoring.
\item Treatment: the intervention variable - binary is fully supported while numeric and multinomial interventions may be supported in the future.
\end{itemize}
DataTable may include an 'ID' column (potentially for stratified cross-validation, etc) and any number of column containing baseline covariates.
...
```{r formatArguments: data structure, eval=FALSE}
...
```{r formatArguments(Intervention = ), eval=FALSE}
ConcreteArgs <- formatArguments(DataTable = obs, EventTime = "time", EventType = "status", 
                                Treatment = "trt", ID = "id", 
...
ConcreteArgs <- formatArguments(DataTable = obs, EventTime = "time", EventType = "status", 
                                Treatment = "trt", ID = "id", 
                                Intervention = ITT, TargetEvent = 1:2, TargetTime = ExtrapolationTime)
```
